    ...
    <section className="print-page mx-auto bg-white p-8 shadow-sm print:shadow-none">
      ...
      <div className="mt-6 space-y-4">
        {invoice.items.map((item) => {
          ...
          return (
            <table key={item.id} className="w-full border-collapse text-sm">
              <thead>
                <tr>
              ...
              <tbody>
                {item.entries.map((entry, idx) => (
                  <tr key={`${item.id}-${entry.id}`}>
                    <td className="print-td text-center">{slipQtyValue(entry.quantity)} 
                      {/* {normalizeEntryType(entry.entry_type || item.entry_type) === 'pieces' ? 'Pcs' : 'Kg'} */}
                    </td>
                  </tr>
                ))}
                <tr>
